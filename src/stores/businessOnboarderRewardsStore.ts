...
interface BusinessOnboarderRewardsState {
  viewState: ViewState;
  data: UserGraphModel[];
  totalAmount: string;
...
export const useBusinessOnboarderRewardsStore = create<BusinessOnboarderRewardsState>((set, get) => ({
  ...
  fetchForDays: async (days, signal) => {
    try {
      ...
      const response = await fetch(
        `${VITE_HD_API_SERVER}/reports/businessOnboarderRewards?days=${days}`,
      ...
      console.error('Failed to fetch businessOnboarderRewards:', e);
    }
  },
}));
